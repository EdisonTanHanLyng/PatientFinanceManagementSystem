...
namespace PFMS_MI04.Services
{
    public class PatientDocService
    {
        ...
        public async Task<bool> uploadDoc(PatientDocument doc)
        {
            return await uploadDocumentRecord(doc);
        }

        public async Task<List<PatientDocument>> getDocs(string patID)
        {
            return await getDocumentsByPatient(patID);
        }
        ...
        public async Task<bool> updateDoc(PatientDoc doc)
        {
        ...
        // Executes an SQL query to get all document entries according to a given patient ID
        private async Task<List<PatientDocument>> getDocumentsByPatient(string patID)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    {
                        comm.CommandText = @"SELECT * FROM patient_documents WHERE patID = @patientID";
                        ...
                        using (MySqlDataReader reader = comm.ExecuteReader()) 
                        {
                            while (reader.Read())
                            {
                                ...
                                doc.patientID = reader["patID"].ToString();
                                docList.Add(doc);
        ...
        private async Task<bool> uploadDocumentRecord(PatientDocument doc)
        {
            bool inserted = false;
            try
            {
        ...
        private async Task<bool> deleteDocument(int docID)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    command.CommandText = @"DELETE FROM patient_documents WHERE docID = @documentID";
        ...
        public async Task<bool> updateDocument(PatientDoc doc)
        {
            bool updated = false;
            try
            {
        ...
        private MySqlCommand docCommandBuilder(MySqlCommand command, PatientDocument doc)
        {
            try
            {
                command.CommandText = @"
                    INSERT INTO patient_documents(patID, docName, docBytes, docDate, docRef, isPreviewable, remarks)
                    VALUES (@patID, @docName, @docBytes, @docDate, @docRef, @isPreviewable, @remarks);
                ";

                command.Parameters.AddWithValue("@patID", doc.patientID);
        ...
        private MySqlCommand docUpdateCommandBuilder(MySqlCommand command, PatientDoc doc)
        {
            try
            {
                command.CommandText = @"
