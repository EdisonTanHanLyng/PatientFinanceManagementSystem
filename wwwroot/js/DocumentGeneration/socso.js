...
document.getElementById('generateExcelButton').addEventListener('click', function (e) {
    ...
    let paymentTo = document.getElementById('paymentTo').value;

    // Collect treatments (Tarikh Dialisis and Kos Dialisis)
    let treatments = [];
    let rows = document.querySelectorAll('.table-wrapper tbody tr:not(.total-row)');
    rows.forEach((row, index) => {
        let nameInput = row.querySelector(`input[name="tarikhRawatan${index + 1}"]`);
        let kosDituntutInput = row.querySelector(`input[name="kosDituntut${index + 1}"]`);
        let kosSuntikanInput = row.querySelector(`input[name="kosSuntikan${index + 1}"]`);
        let catatanInput = row.querySelector(`input[name="catatan${index + 1}"]`);

        // Check if the input element exists, then get the value,
        let tarikhRawatan = nameInput ? (nameInput.value || "") : "";
        let kosDituntut = kosDituntutInput ? (kosDituntutInput.value || 0) : 0;
        let kosSuntikan = kosSuntikanInput ? (kosSuntikanInput.value || 0) : 0;
        let catatan = catatanInput ? (catatanInput.value || "") : "";

        treatments.push({
            TreatmentDate: tarikhRawatan,
            DialysisCost: kosDituntut,
            MedicineCost: kosSuntikan,
            Status: catatan
        });
    ...
    fetch('/DocumentGen/GenerateSOCSO', {
        method: 'POST',
        headers: {
...
function recalculateTotal() {
    let overallTotalDituntut = 0;
    let overallTotalSuntikan = 0;
    let overallTotal = 0;
    ...
    rows.forEach((row) => {
        let kosDituntutInput = row.querySelector(`input[name^="kosDituntut"]`);
        let kosSuntikanInput = row.querySelector(`input[name^="kosSuntikan"]`);

        // Parse the input values, default to 0 if they are empty
        let kosDituntut = parseFloat(kosDituntutInput.value) || 0;
        let kosSuntikan = parseFloat(kosSuntikanInput.value) || 0;

        // Accumulate totals
        overallTotalDituntut += kosDituntut;
        overallTotalSuntikan += kosSuntikan;

    });
    overallTotal += overallTotalDituntut + overallTotalSuntikan;
    // Update the overall total cells
    document.getElementById('totalRawatan').innerText = overallTotalDituntut.toFixed(2);
    document.getElementById('totalSuntikan').innerText = overallTotalSuntikan.toFixed(2);
    document.getElementById('totalKos').innerText = overallTotal.toFixed(2);
}

// Add event listeners to all Kos Dialisis and Kos EPO cells
document.querySelectorAll(`input[name^="kosDituntut"], input[name^="kosSuntikan"]`).forEach(input => {
...
function confirmSelection() {
    sessionStorage.setItem("currentSponsor", "SOCSO");
    sessionStorage.setItem("currentPage", "SOCSO");
...
function autoFillDetails() {
    ...
    if (selectedPatient) {
        ...
        document.getElementById("paymentTo").value = 'SOCSO';
...
document.getElementById('fillBtn').addEventListener('click', function () {
    ...
    // Get the values from the pRawatan and pSuntikan inputs
    const pRawatanValue = parseFloat(document.getElementById('pRawatan').value);
    const pSuntikanValue = parseFloat(document.getElementById('pSuntikan').value);

    // Check if the values are 0
    if (pRawatanValue === 0 || pSuntikanValue === 0) {
        alert("Prices are 0.");
        return;
    }
    ...
    rows.forEach((row, index) => {
        ...
        if (tarikhRawatanInput && tarikhRawatanInput.value.trim() !== "") {
            let kosRawatanInput = row.querySelector(`input[name="kosDituntut${index + 1}"]`);
            let kosSuntikanInput = row.querySelector(`input[name="kosSuntikan${index + 1}"]`);

            // Set both kos rawatan and kos suntikan to the values from the inputs
            if (kosRawatanInput) kosRawatanInput.value = pRawatanValue.toFixed(2);
            if (kosSuntikanInput) kosSuntikanInput.value = pSuntikanValue.toFixed(2);
        }
    });
...
document.querySelectorAll('input[name^="pRawatan"], input[name^="pSuntikan"]').forEach(input => {
...
async function updatePrice() {
    const sponsorCode = "SOCSO";
    const dialysisCost = document.getElementById("pRawatan").value;
    const epoCost = document.getElementById("pSuntikan").value;
...
async function getPrice() {
    ...
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('pSuntikan').value = data.data.epoCost;
        } else {
